...
fprintf('Running makepgfplots_figure21.m; type: %s\n',type);


colors{1} = '{SPECorange}';
colors{2} = '{SPECorange!80!SPECred}';
colors{3} = '{SPECorange!60!SPECred}';
colors{4} = '{SPECorange!40!SPECred}';
colors{5} = '{SPECorange!20!SPECred}';
colors{6} = '{SPECred}';
colors{7} = '{SPECred!80!black}';
colors{8} = '{SPECred!60!black}';
colors{9} = '{SPECred!40!black}';
colors{10} = '{SPECred!20!black}';
colors{11} = '{black}';
colors{12} = '{black!80!SPECgreen}';
colors{13} = '{black!60!SPECgreen}';
colors{14} = '{black!40!SPECgreen}';
colors{15} = '{black!20!SPECgreen}';
colors{16} = '{SPECgreen}';
colors{17} = '{SPECgreen!90!SPECblue}';


if (taylor)
	
	if (usevpa)
		filename = sprintf('exp_%s_ksdensity_split_taylor_%d_%d_%f_%d_vpa.tex',type,n,p,t0,example);
	else
		filename = sprintf('exp_%s_ksdensity_split_taylor_%d_%d_%f_%d.tex',type,n,p,t0,example);
	end

else
	
	if (usevpa)
		filename = sprintf('exp_%s_ksdensity_split_chebyshev_%d_%d_%f_%f_%d_vpa.tex',type,n,p,T2(1),T2(2),example);
	else
		filename = sprintf('exp_%s_ksdensity_split_chebyshev_%d_%d_%f_%f_%d.tex',type,n,p,T2(1),T2(2),example);
	end
...
for kk = 1:length(mus)
	...
	for ii = 1:n
		...
		smu = [smu; mus(kk)+ zeros(size(emu(1:(timesteps-1)*realizations,ii)))];
...
fprintf(figout,'\\beginpgfgraphicnamed{figure-split-%d-%d-%d}\n',n,p,round(t0*100));
...
for qq=1:length(mus)
	fprintf(figout,'\n');
	fprintf(figout,'    \\addplot[%s, very thick, only marks]\n',colors{qq});
	fprintf(figout,'    coordinates{%%\n');
	fprintf(figout,'    (%e,%e)%%\n',mus(qq),0);
	fprintf(figout,'    };%%\n');
	th1 = t(smu==mus(qq));
	th1 = th1(th1<T2(2) & th1>T2(1));
	if (~isempty(th1))
		%th1
		fprintf(figout,'    \\addplot[%s, thick]\n',colors{qq});
		fprintf(figout,'    coordinates{%%\n');
		[fpart,xipart,bwpart] = mvksdensity(th1,'NumPoints',1001,'bandwidth',bw);
		for kk = 1:length(xipart)
			fprintf(figout,'    (%e,%e)%%\n',xipart(kk),1001/10001*(length(th1)/length(ts))*(sum(f)/sum(fpart))*fpart(kk));
		end
		fprintf(figout,'    };%%\n');
	else
		fprintf('th1 is empty for mu(%d) = %f\n',qq,mus(qq));
	end
...
if (taylor)
	if (usevpa)||(usesingle)
		fprintf(figout,'\\label{fig11:example1vpa_splitx_%d_%d_%d_%3.2f_%d}%%\n',taylor,n,p,t0,example);
	else
		fprintf(figout,'\\label{fig11:example1_split_%d_%d_%d_%3.2f_%d}%%\n',taylor,n,p,t0,example);
	end		
else	
	if (usevpa)||(usesingle)
		fprintf(figout,'\\label{fig11:example1vpa_split_%d_%d_%d_%3.2f_%d}%%\n',taylor,n,p,T2(1),example);
	else
		fprintf(figout,'\\label{fig11:example1_split_%d_%d_%d_%3.2f_%d}%%\n',taylor,n,p,T2(1),example);
	end		
end
